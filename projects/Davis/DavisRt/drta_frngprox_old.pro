...
;    pmap = php.xmag*xx*xx + php.ymag*yy*yy + php.xymag*xx*yy
    ...
    if php.lambda ne 632.8 then begin
    	pmap = pmap*(632.8/php.lambda)
    endif
...
pro frng_spx, refim, img, php, nchan, ordwin, r, culz, spectrum, tstamp = tstamp, zerph=zerph, scan_range=scan_range
  ...
  if strupcase(!d.name) ne 'Z' and php.fplot ne 0 then window, 0, xsize=php.nx, ysize=php.ny, xpos=2, ypos=2, title="Scanning..."
...
;      php.minord = php.minord + 1./float(nchan)
  ...
  if strupcase(!d.name) ne 'Z' and php.fplot ne 0 then begin
...
pro frng_newspex, img, php, nchan, spx, ordwin, culz
    ...
    for i=0,n_elements(pmap(*,0))-1 do begin
        for j=0,n_elements(pmap(0,*))-1 do begin
            if pmap(i,j) ge owin(0) and pmap(i,j) lt owin(1) then begin
