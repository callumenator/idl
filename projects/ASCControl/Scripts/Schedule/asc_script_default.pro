...
function ASC_Script_Default, info, reset=reset

  COMMON ASC_Script_Default_Common, block0_flag
  ...
  ;if (HourUT() gt 3.5) and (HourUT() lt 15.0) then begin  
  if SolarZenithAngle(info) lt -8 then begin
    ...
    if block0_flag eq 0 then begin
      ...
      queue = [$;\\ 6300
               ...
               'grab_frame, numframes = 1, save_raw = 1, ' + $
                   'inter_frame_delay = 0.05, postprocess = ASC_ImagePostProcess', $
               ;\\ 5577
               'filter, 2', $
  ...
  ;if (HourUT() lt 3.45) or (HourUT() gt 15.05) then begin
  if SolarZenithAngle(info) gt -7.7 then begin 
