...
'SRF':	read_srf, name, img, r, g, b
'JPG':	begin
	  read_jpeg, name, img, c, colors=256,/dither,/two_pass
	  if (size(c))(0) eq 1 then begin	; Gray scale.
	    r = c
	    g = c
	    b = c
...
'TIF':	begin
	  ...
	  sz = size(img)
	  if sz(0) eq 0 then begin	; Planarconfig=2
	    xmess,'Loading 24 bit image . . .',wid=wid,/nowait
	    rr = r
	    gg = g
	    bb = b
;	    img = color_quan(rr,gg,bb,r,g,b)
	    img = color_quanj(rr,gg,bb,r,g,b,/dither)
	    img = reform(img)
...
;	if n_elements(smax) eq 0 then smax = round(0.9*ssz)
	mx = smax(0)
	my = smax(1)
	sz = size(img)
	nx = sz(1)
