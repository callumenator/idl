...
;-
;-------------------------------------------------------------
	pro sunaltazi, time, lng, lat, azi, alt, zone=zone, $
	...
	if dt_tm_chk(time(0)) eq 0 then begin	; If array assume all same typ.
	  if datatype(time) ne 'DOU' then begin
	    print,' Error in sunaltazi: time must be either a date/time string'
