...
pro ASC_LoadCameraSetting_Readback, exposureTime = exposureTime, $
									emGain = emGain, $
									vsSpeed = vsSpeed, $
									hsSpeed = hsSpeed, $
									readMode = readMode, $
	...
	if keyword_set(exposureTime) then begin
		;\\ Get the actual exp time used
			andor_camera_driver, info.external_dll, 'uGetAcquisitionTimings', 0, out, /auto_acq
			info.camera_settings.exptime_use = out.exposure
	endif

	if keyword_set(emGain) then begin
		;\\ Get the actual gain used by the camera
			andor_camera_driver, info.external_dll, 'uGetEMCCDGain', 0, emgain, res, /auto_acq
			info.camera_settings.emgain_use = emgain
	endif

	if keyword_set(triggerMode) then begin
		trig_modes = ['Internal', 'External', 'External Start']
		if info.camera_settings.triggermode ge 0 and $
		   info.camera_settings.triggermode lt n_elements(trig_modes) then begin
			info.camera_settings.triggermode_str = trig_modes[info.camera_settings.triggermode]
		endif else begin
			info.camera_settings.triggermode_str = 'Error'
		endelse
	endif
	...
	if keyword_set(acqMode) then begin
		...
		if info.camera_settings.acqmode ge 0 and $
		   info.camera_settings.acqmode lt n_elements(acq_modes) then begin
			info.camera_settings.acqmode_str = acq_modes[info.camera_settings.acqmode]
		endif else begin
			info.camera_settings.acqmode_str = 'Error'
		endelse
	endif
	...
	if keyword_set(readMode) then begin
		...
		if info.camera_settings.readmode ge 0 and $
		   info.camera_settings.readmode lt n_elements(read_modes) then begin
			info.camera_settings.readmode_str = read_modes[info.camera_settings.readmode]
		endif else begin
			info.camera_settings.readmode_str = 'Error'
		endelse
	endif

	if keyword_set(vsSpeed) then begin
		vshift = (*info.camera_caps).vsspeeds[info.camera_settings.vsspeedi]
		info.camera_settings.vsspeed = vshift
	endif

	if keyword_set(hsSpeed) then begin
		;\\ Get the actual shift speed
			andor_camera_driver, info.external_dll, 'uGetHSSpeed', $
						 {adchannel:info.camera_settings.adchannel, $
						  outputamp:info.camera_settings.outamp, $
						  hsindex:info.camera_settings.hsspeedi}, $
						 hshift, res, /auto_acq

			info.camera_settings.hsspeed = hshift
	endif
end


;\\ Helper function for setting individual camera settings, for example
;\\ after camera has been initialized and ASC_LoadCameraSettings use to
...
pro ASC_LoadCameraSetting,  camera_dll, $
  ...
  COMMON ASC_Control, info, gui, log
	...
	;\\ Call the driver for the appropriate setting
		...
		if keyword_set(readMode) then begin
			...
			info.camera_settings.readmode = readMode
			_res = [_res, result]

			ASC_LoadCameraSetting_Readback, /readMode
		endif

		if keyword_set(acqMode) then begin
			Andor_Camera_Driver, camera_dll, 'uSetAcquisitionMode', acqMode, out, result, /auto_acq
			info.camera_settings.acqmode = acqMode
			_res = [_res, result]

			ASC_LoadCameraSetting_Readback, /acqMode
		endif	
		
		if keyword_set(triggerMode) then begin
			Andor_Camera_Driver, camera_dll, 'uSetTriggerMode', triggerMode, out, result, /auto_acq
			info.camera_settings.triggerMode = triggerMode
			_res = [_res, result]

			ASC_LoadCameraSetting_Readback, /triggerMode
		endif
    ...
    if size(preAmpGainIndex, /type) ne 0 then begin
      ...
      _res = [_res, result]      
		...
		if size(baselineClamp, /type) ne 0 then begin
			...
			info.camera_settings.baselineClamp = baselineClamp
			_res = [_res, result]
		endif
		...
		if size(coolerOn, /type) ne 0 then begin
			...
			endelse
			info.camera_settings.coolerOn = coolerOn
			_res = [_res, result]
		endif
		...
		if keyword_set(imageMode) then begin 
			Andor_Camera_Driver, camera_dll, 'uSetImage', $
			 [imageMode.xbin, imageMode.ybin, imageMode.xpixstart, imageMode.xpixstop, $
			  ...
			  info.camera_settings.imageMode = imageMode
			_res = [_res, result]			
		endif

		if size(vsSpeedindex, /type) ne 0 then begin
			Andor_Camera_Driver, camera_dll, 'uSetVSSpeed', vsSpeedIndex, out, result, /auto_acq
			info.camera_settings.vsspeedi = vsSpeedIndex
			_res = [_res, result]

			ASC_LoadCameraSetting_Readback, /vsSpeed
		endif
		...
		if size(hsSpeedindex, /type) ne 0 then begin
			...
			info.camera_settings.hsspeedi = hsSpeedIndex
			_res = [_res, result]

			ASC_LoadCameraSetting_Readback, /hsSpeed
		endif

		;\\ If a script is supplied, use that to update the camera settings
		if keyword_set(settings_script) then begin

			cam_settings = info.camera_settings
			...
			;\\ Upload the settings to the camera
				...
				info.camera_settings = cam_settings
				_res = [_res, result]				
