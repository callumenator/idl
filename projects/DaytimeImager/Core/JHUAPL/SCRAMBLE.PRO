...
;-------------------------------------------------------------
	...
	;--------  Number of indices to scramble  -------
	  ...
	  ir = indgen(n)	   ; Array to scramble.
	endelse
 
	r = intarr(n)		   ; Make output array space.
 
	for i = n-1,0,-1 do begin  ; Loop thru unscrambled list backwards.
	  rnd = randomu(k)
	  j = fix(rnd*(i+1))	   ; Random number from 0 to I.
