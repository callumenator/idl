...
pro Andor_Camera_Driver_GUI_Init, base_widget=base_widget, $
	...
	common AndorCameraDriverGUI, gui, misc
	...
	;\\ CAMERA STATUS, TEMPERATURE, ETC., WRITE SETTINGS BUTTON
		...
		write_label = widget_button(stat_base, value = 'Write Settings to Profile', font=font, uval={type:'settings_write_asc'})
		write_label = widget_button(stat_base, value = 'Read Settings from Profile', font=font, uval={type:'settings_read_asc'})


	;\\ INIT AND SHUTDOWN BUTTONS
		btn_base = widget_base(tp_base, col=4)
		btn = widget_button(btn_base, value = 'Init Camera', font=font, uval={type:'cam_init'})
		btn = widget_button(btn_base, value = 'Shutdown Camera', font=font, uval={type:'cam_shutdown'})
		btn = widget_button(btn_base, value = 'Abort Acq.', font=font, uval={type:'acq_abort'})
		btn = widget_button(btn_base, value = 'Start Acq.', font=font, uval={type:'acq_start'})
	...
	if size(uval, /type) eq 8 then begin
		case uval.type of
			'acq_abort':begin
				...
				Andor_Camera_Driver_GUI_UpdateLog, 'Abort Acq: ' + res
			end
			'acq_start':begin
				Andor_Camera_Driver, misc.dll, 'uStartAcquisition', 0, out, res
				Andor_Camera_Driver_GUI_UpdateLog, 'Start Acq: ' + res
			end
			'cam_init': begin
				Andor_Camera_Driver, misc.dll, 'uInitialize', 0, out, res, /auto_acq
				Andor_Camera_Driver_GUI_UpdateLog, 'Initialize: ' + res
			end
			'cam_shutdown': begin
				Andor_Camera_Driver, misc.dll, 'uShutdown', 0, out, res, /auto_acq
				Andor_Camera_Driver_GUI_UpdateLog, 'Shutdown: ' + res
			end
