pro sdi3k_batch_wind_summary, ncfile, culz, wscale
...
;    obj_edt, hwm, title='HWM Parameters'
    ...
    ap    = fltarr(7)
    ap(0) = hwm.ap
    magrot= !dtor*(mm.oval_angle + mtr.rotation_from_oval)
...
vz = spekfits.velocity(0)*mm.channels_to_velocity

;drftfit = poly_fit((spekfits.start_time + spekfits.end_time)/2 - spekfits(0).start_time, spekfits.velocity(0), 4, measure_errors=1.+spekfits.sigma_velocity(0), yfit=drift, /double)
...
xtn     = dt_tm_mk(js2jd(0d)+1, ttvals, format='h$:m$')
    ...
    plot, timlimz, [-wrange, wrange], /noerase, color=culz.black, /nodata, /xstyle, /ystyle, $
          ...
          mc_oploterr, (windpars.start_time + windpars.end_time)/2, zonal, sigzon, $
                        bar_color=culz.ash, thick=4, symbol_color=culz.slate, line_color=culz.black, /two, psym=6, symsize=.3
    ...
    plot, timlimz, [-wrange, wrange], /noerase, color=culz.black, /nodata, /xstyle, /ystyle, $
          ...
          mc_oploterr, (windpars.start_time + windpars.end_time)/2, merid, sigmer, $
                        bar_color=culz.ash, thick=2, symbol_color=culz.rose, line_color=culz.red, /two, psym=6, symsize=.3
        tm       = timlimz(0) + 0.97*deltime
        ym       = 0.8*wrange
        for idz=-1,1 do begin
            thk = 1 + (idz eq 0)
