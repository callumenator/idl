...
pro sdi3k_simple_plotter, filename, plotarr, canvas_size=canvas_size, what_plot=what_plot, time_smoothing=tsm, space_smoothing=ssm, yrange=yrange, quality_filter=quality_filter

    if not(keyword_set(what_plot)) then begin
       plotsel = ['LOSWIND',  'TEMPERATURE',     'INTENSITY',  'RGBMAPS', $
                  'WINDMAPS', 'XY_TEMPERATURE',  'MEAN_WINDS', 'WIND_GRADIENTS']
    ...
    sdi3k_read_netcdf_data, filename, metadata=mm, winds=winds, spekfits=spekfits, windpars=windpars, zonemap=zonemap, zone_centers=zone_centers
...
XY_TEMPERATURE:
    scale = {time_range: [0., 1.], yrange: tprscale, auto_scale: 0}
    geo   = {xsize:  xsize, ysize: ysize}
    ...
    sdi3k_tseries_plotter, tlist, tcen, mm, spekfits, tsplot_settings, culz, quality_filter=quality_filter, pastel=pastel
    goto, PLOTZ_DONE
...
;    yinfo.charsize = 1.8
    ...
    zondata = {x: tcen(jlo:jhi), y: windpars(jlo:jhi).mag_zonal_wind}
    mc_npanel_plot,  layout, yinfo, zondata
    yinfo.title = 'Magnetic Meridional!C !CWind [m/s]'
    zondata = {x: tcen(jlo:jhi), y: windpars(jlo:jhi).mag_meridional_wind}
    mc_npanel_plot,  layout, yinfo, zondata
