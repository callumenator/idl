pro sdi3k_tseries_plotter, tlist, tcen, mm, resarr, tsplot_settings, culz, sigma_name=signame, units_name=unitname, colors=colors

    if not(keyword_set(colors)) then colors = {bar_color: culz.blue, line_color: culz.black, symbol_color: culz.red}
...
;          oploterr, tt, yvals, sigvals, 3
           ...
           bar_color=colors.bar_color, line_color=colors.line_color, errthick=tsplot_settings.style.line_thickness, $
...
;                  oplot,  tmx, msis_vals, thick=3, linestyle=1, color=host.colors.rose
            ...
            xyouts, t1, y1, hlab +' km', charsize=((tsplot_settings.style.charsize - 4) > 1), color=culz.red, align=1, charthick=(tsplot_settings.style.charthick-1)>1
         endif
        endfor
