...
pro sdi_monitor_timeseries
	...
	;\\ Things to plot...
		show = [{tag:'width', site:'all', wavelength:'6300', range:[600.,1400.], zone:-1, title:'630nm Temperature (K)', thick:1.5}, $
				{tag:'width', site:'all', wavelength:'5577', range:[200., 700.], zone:-1, title:'558nm Temperature (K)', thick:1.5}, $
				{tag:'snr', site:'all', wavelength:'6300', range:[0.,1E4], zone:-1, title:'630nm SNR/Scan', thick:1.5}, $
				{tag:'snr', site:'all', wavelength:'5577', range:[0.,1E5], zone:-1, title:'558nm SNR/Scan', thick:1.5}, $
	...
	;\\ Plot median temperature for each site
		site_colormap = {site:['HRP', 'PKR', 'MAW'], $
						 color:[100, 150, 190], $
						 ctable:[39, 39, 39] }
	...
	;\\ Find timeseries save files
		ts_files = file_search(global.home_dir + '*_timeseries.idlsave', count = n_series)
	...
	;\\ Split up the page
		bounds = split_page(n_elements(show), 1, bounds=[.05, .05, .98, .99], row_gap=.08)
		...
		for p = 0, n_elements(show) - 1 do begin
			...
			for k = 0, n_series - 1 do begin
					...
					'position': begin
						...
						for kk = 0, nb - 1 do begin
							...
							if (i1-i0) gt 3 then begin
								...
								if ngood gt 15 then begin
									coeffs = poly_fit((xaxis[i0:i1])[good], (parameter[i0:i1])[good], 3, yfit=curve)
									parameter[i0:i1] -= interpol(curve - curve[0], (xaxis[i0:i1])[good], (xaxis[i0:i1]))
								endif
							endif
				...
				if n_elements(series) gt 3 then begin
					loadct, 0, /silent
					errplot, xaxis, parameter - sdevs, parameter + sdevs, color=90, width=.00001, noclip=0
			...
			if nels(sites_used) gt 0 then begin
				...
				for k = 0, n_elements(u_sites) - 1 do begin
					...
					xyouts, time_range[0] + k*0.05*(time_range[1]-time_range[0]), $
							...
							color=colors_used[index], align=-1.5
				endfor
				!p.font = -1
			endif
