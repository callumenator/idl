...
; CLFIND.PRO
;-----------------------------------------------------------------------------
; Find clumps in a x-y-v fits data cube
...
;
; needs: clfind.cb      ; common block
; usage: .run,clfind    ; to compile all procedures
;-----------------------------------------------------------------------------
;...................... START DEFREG ......................
...
; read in data + header
data=readfits(file,header,/silent)
readhd,header
nx=long(naxis1)
ny=long(naxis2)
nv=long(naxis3)
...
for nwork=nlev-1,0,-1 do begin
  ...
  ncl=ncl+nnew
	...
	pcnt = (1 - float(nwork+1)/float(nlev))*100.
	print, pcnt
	wait, 0.001
endfor
