; $Id: polyfitw.pro,v 1.2 1996/12/17 20:21:56 ali Exp $
...
;
YFIT = DBLARR(N)+C[0] ; ONE-SIGMA ERROR ESTIMATES, INIT
FOR K = 1, NDEGREE DO YFIT = YFIT + C[K]*(X^K) ; SUM BASIS VECTORS
;
VAR = DOUBLE(TOTAL((YFIT-Y)^2 )/(N-M)) ; VARIANCE ESTIMATE, UNBIASED
;
;
SIGMA=DOUBLE(SQRT(VAR))
YBAND = DBLARR(N) + A[0,0]
Z=DBLARR(N)+1.
