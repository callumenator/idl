...
;==========================================================================================
;
; >>>> McObject Class: sdi2k_math_skymap
;
; This file contains the McObject method code for sdi2k_math_skymap objects:
;
; Mark Conde Fairbanks, October 2000.
;
; >>>> end comments
; >>>> begin declarations
;         menu_name = Skymap Plotter
;        class_name = sdi2k_math_skymap
;       description = SDI Analysis - Plot Skymaps
...
pro sdi2k_math_skymap_new, instance, dynamic=dyn, creator=cmd
;---First, properties specific to this object:
    common skymap_resarr, resarr, smootharr
    cmd = 'instance = {sdi2k_math_skymap, '
    cmd = cmd + 'specific_cleanup: ''sdi2k_math_skymap_specific_cleanup'', '
    skymap_behavior = {skymap_behavior, prompt_for_filename: 1, $
                          menu_configurable: 0, $
                              user_editable: [1]}
    skymap_scale = {skymap_scale, auto_scale: 0, $
                                      yrange: [0., 80.], $
                           menu_configurable: 1, $
                               user_editable: [0,1]}
    skymap_geom  = {skymap_geom, viewing_from_above: 0, $
                              radius_maps_to_distance: 0, $
                                       north_rotation: 0, $
                                  rainbow_intensities: 0, $
                                    menu_configurable: 1, $
                                        user_editable: [0,1,2,3]}     
    rex = [0, n_elements(resarr)-1]
    smoothing = {skymap_smoothing,     wind_time_smoothing: 0.7, $
                                    wind_spatial_smoothing: 0.05, $
                                temperature_time_smoothing: 1., $
                             temperature_spatial_smoothing: 0.08, $
                                  intensity_time_smoothing: 1.1, $
                               intensity_spatial_smoothing: 0.05, $
                                menu_configurable: 1, $
                                    user_editable: [0,1,2,3,4,5]}

    cmd = cmd + 'behavior: skymap_behavior, '
    cmd = cmd + 'scale: skymap_scale, '
    cmd = cmd + 'smooth_settings: smoothing, '
    cmd = cmd + 'black_bgnd: 1, '
    cmd = cmd + 'parameter: 6, '
    cmd = cmd + 'map_view: skymap_geom, '
    cmd = cmd + 'records: rex, '
...
;==========================================================================================
; This is the event handler for events generated by the sdi2k_math_skymap object:
pro sdi2k_math_skymap_event, event
    ...
    widget_control, sidx, get_uvalue=skymap_settings
    if widget_info(event.id, /valid_id) and skymap_settings.automation.show_on_refresh then widget_control, event.id, /show

;---Check for a timer tick:
    if tag_names(event, /structure_name) eq 'WIDGET_TIMER' then begin
       sdi2k_math_skymap_tik, info.wtitle
       if skymap_settings.automation.timer_ticking then widget_control, sidx, timer=skymap_settings.automation.timer_interval
       return
    endif
...
;---Get the menu name for this event:
    ...
    if menu_item eq 'View|Time Chooser' then begin
       ...
       skymap_settings.records(0) = choice.index
       mcchoice, 'End time:', tlist, choice
       skymap_settings.records(1) = choice.index
       widget_control, sidx, set_uvalue=skymap_settings
    endif

    if menu_item eq 'View|Smooth Winds' then begin
       wot = resarr(skymap_settings.records(0):skymap_settings.records(1)).velocity
       print, 'Smoothing Winds in Time...'
       sdi2k_timesmooth_fits, wot, skymap_settings.smooth_settings.wind_time_smoothing, /progress
       print, 'Smoothing Winds in Space...'
       sdi2k_spacesmooth_fits, wot, skymap_settings.smooth_settings.wind_spatial_smoothing, /progress
       smootharr(skymap_settings.records(0):skymap_settings.records(1)).velocity = wot
    endif

    if menu_item eq 'View|Smooth Temperatures' then begin
       wot = resarr(skymap_settings.records(0):skymap_settings.records(1)).temperature
       print, 'Smoothing Temperatures in Time...'
       sdi2k_timesmooth_fits, wot, skymap_settings.smooth_settings.temperature_time_smoothing, /progress
       print, 'Smoothing Temperatures in Space...'
       sdi2k_spacesmooth_fits, wot, skymap_settings.smooth_settings.temperature_spatial_smoothing, /progress
       smootharr(skymap_settings.records(0):skymap_settings.records(1)).temperature = wot
    endif

    if menu_item eq 'View|Smooth Intensities' then begin
       wot = resarr(skymap_settings.records(0):skymap_settings.records(1)).intensity
       print, 'Smoothing Intensities in Time...'
       sdi2k_timesmooth_fits, wot, skymap_settings.smooth_settings.intensity_time_smoothing, /progress
       print, 'Smoothing Intensities in Space...'
       sdi2k_spacesmooth_fits, wot, skymap_settings.smooth_settings.intensity_spatial_smoothing, /progress
       smootharr(skymap_settings.records(0):skymap_settings.records(1)).intensity = wot
    endif

    if menu_item eq 'View|Toggle Background Color' then begin
       skymap_settings.black_bgnd = 1 - skymap_settings.black_bgnd
       widget_control, sidx, set_uvalue=skymap_settings
    endif

    if menu_item eq 'View|Plot Parameter' then sdi2k_skymap_getpar, info.wtitle
    sdi2k_math_skymap_plot, info.wtitle
...
pro sdi2k_math_skymap_tik, wtitle, redraw=redraw, _extra=_extra
    sdi2k_math_skymap_plot, wtitle
@sdi2kinc.pro
end

pro sdi2k_skymap_getpar, wtitle
@sdi2kinc.pro
    common skymap_resarr, resarr, smootharr
    ...
    skymap_settings.parameter = skies(choice.index)
    case choice.name of
       'Velocity':    begin
                      skymap_settings.scale.auto_scale = 0
                      skymap_settings.scale.yrange = [-400., 400.]
                      end
       'Temperature': begin
                      skymap_settings.scale.auto_scale = 0
                      skymap_settings.scale.yrange = [600., 1800.]
                      end
       'Intensity':   begin
                      skymap_settings.scale.auto_scale = 0
                      skymap_settings.scale.yrange = [0., 200.]
                      end
        else:         begin
                      skymap_settings.scale.auto_scale = 1
                      end
    endcase
    widget_control, sidx, set_uvalue=skymap_settings
end
...
pro sdi2k_math_skymap_plot, wtitle
@sdi2kinc.pro
    common skymap_resarr, resarr, smootharr
    common skymap_tlist,  tlist, datestr
...
;   the plot program itself:
    ...
    sdi2k_sky_mapper, tlist, tcen, datestr, smootharr, skymap_settings, $
                      map_view=skymap_settings.map_view.viewing_from_above, $
                      azimuth_rotation=skymap_settings.map_view.north_rotation, rainbow_intensities=skymap_settings.map_view.rainbow_intensities
...
;==========================================================================================
;   Cleanup jobs:
pro sdi2k_math_skymap_specific_cleanup, widid
@sdi2kinc.pro
    close, /all
...
pro sdi2k_math_skymap_autorun, instance
@sdi2kinc.pro
    common skymap_resarr, resarr, smootharr
    common skymap_tlist, tlist, datestr
    ...
    nv = n_elements(resarr)
    marg = 16 < nv/2 - 1
    if abs(host.operation.calibration.sky_wavelength -589.0) gt 5. then rex = indgen(n_elements(resarr)) else begin
...
;    if n_elements(zone_map) lt 1 then sdi2k_build_zone_map, canvas_size = [0.95*min(box), 0.95*min(box)], /map_project
    ...
    mc_menu, extra_menu, 'View',                    1, event_handler='sdi2k_math_skymap_event', /new
    mc_menu, extra_menu, 'Plot Parameter',          0, event_handler='sdi2k_math_skymap_event'
    mc_menu, extra_menu, 'Time Chooser',            0, event_handler='sdi2k_math_skymap_event'
    mc_menu, extra_menu, 'Smooth Winds',            0, event_handler='sdi2k_math_skymap_event'
    mc_menu, extra_menu, 'Smooth Temperatures',     0, event_handler='sdi2k_math_skymap_event'
    mc_menu, extra_menu, 'Smooth Intensities',      0, event_handler='sdi2k_math_skymap_event'
    mc_menu, extra_menu, 'Toggle Background Color', 0, event_handler='sdi2k_math_skymap_event'
    mc_menu, extra_menu, 'Redraw',         2, event_handler='sdi2k_math_skymap_event'
    mnu_xwindow_autorun, instance, topname='sdi2ka_top', extra_menu=extra_menu

    sdi2k_math_skymap_plot, instance.description
end

;==========================================================================================
; This is the (required) class method for creating a new instance of the sdi2k_math_skymap object. It
