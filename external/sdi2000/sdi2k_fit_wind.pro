...
pro sdi2k_fit_wind, resarr, dvdx_zero=dvdx_zero, windfit, src_hgt
@sdi2kinc.pro
  ...
  if abs(host.operation.calibration.sky_wavelength - 630.03) lt 1. then src_hgt = 240000.
  if abs(host.operation.calibration.sky_wavelength - 840.)   lt 1. then src_hgt = 87000.
  if abs(host.operation.calibration.sky_wavelength - 557.7)  lt 1. then src_hgt = 120000.
...
;-Calculate the zone angular radius and bearing values analytically:
       ...
       for j=1,nz-1 do begin
           ...
           ridx(j) = ridx(j-1)
           sidx(j) = sidx(j-1) + 1
           if rcnt eq host.operation.zones.sectors(ridx(j-1)) then begin
              ridx(j) = ridx(j-1) + 1
              sidx(j) = 0
              rcnt = 0
           endif
           if ridx(j) eq 0 then begin
              ...
              rad(j)   = (host.operation.zones.ring_radii(ridx(j)) + host.operation.zones.ring_radii(ridx(j)-1))/2
              theta(j) = (sidx(j) + 0.5)*360/host.operation.zones.sectors(ridx(j))
           endelse
       endfor
...
;           posarr(*,rcd) = posarr(*,rcd) - posarr(0,rcd)*cos(rad)
           ...
           ng      = 0
           goods   = where(resarr(rcd).signal2noise gt -1, ng)
...
;               fouriers(rcd, rng, 0, 1) = fouriers(rcd, rng, *, 1)*2
               fouriers(rcd, rng, 1, 0) = fouriers(rcd, rng, 1, 0)*2
               fouriers(rcd, rng, 1, 1) = fouriers(rcd, rng, 1, 1)*2
           endfor
       endfor
...
;           denom = (abs(ulosarr(*,rcd)*sin(rad))+1.)^2
           ...
           denom = denom + (abs(resarr(rcd).sigma_velocity) + 5.)^2
           chisqr(rcd) = total(sqdif(5:*)/denom(5:*))/(nz - 12)
       endfor
       
;------Construct a structure to hold the results of the fitting:
       latfac = cos(!dtor*host.operation.header.latitude)
       zlat   = host.operation.header.latitude  + 0.001*yy/111.192
       zlon   = host.operation.header.longitude + 0.001*xx/(latfac*111.192)

       windfit =  {zonal_wind: zonal, $
              meridional_wind: -merid, $
                vertical_wind: resarr.velocity(0), $
...
;      the zone lats and lons. We want these to be expressed in geographic, no 
;      geomagnetic coordinates.
       theta  = !pi/2 + (theta + 28.5*!dtor)
       nw     = 0
