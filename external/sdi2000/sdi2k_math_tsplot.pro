...
;==========================================================================================
;
; >>>> McObject Class: sdi2k_math_tsplot
;
; This file contains the McObject method code for sdi2k_math_tsplot objects:
;
; Mark Conde Fairbanks, November 2000.
;
; >>>> end comments
; >>>> begin declarations
;         menu_name = Time Series Plotter
;        class_name = sdi2k_math_tsplot
;       description = SDI Analysis - Time Series
...
pro sdi2k_math_tsplot_new, instance, dynamic=dyn, creator=cmd
;---First, properties specific to this object:
    cmd = 'instance = {sdi2k_math_tsplot, '
    cmd = cmd + 'specific_cleanup: ''sdi2k_math_tsplot_specific_cleanup'', '    
    tsplot_behavior = {tsplot_behavior, prompt_for_filename: 1, $
                           menu_configurable: 1, $
                               user_editable: [0]} 
    tsplot_scale = {tsplot_scale, auto_scale: 1, $
                                      yrange: [0., 1.], $
                                  time_range: [0., 1.], $
                           menu_configurable: 1, $
                               user_editable: [0,1,2]} 
    tsplot_msis  = {tsplot_msis,   plot_msis: 1, $
                                      f10pt7: 200., $
                                          ap: 15., $
                                 msis_height: 240., $
                           menu_configurable: 1, $
    ...
    cmd = cmd + 'behavior: tsplot_behavior, '
    cmd = cmd + 'scale: tsplot_scale, '
    cmd = cmd + 'parameter: 4, '
    cmd = cmd + 'zones: "Zenith", '
    cmd = cmd + 'msis: tsplot_msis, '
...
;==========================================================================================
; This is the event handler for events generated by the sdi2k_math_tsplot object:
pro sdi2k_math_tsplot_event, event
    widget_control, event.top, get_uvalue=info
    wid_pool, 'Settings: ' + info.wtitle, widx, /get
    if not(widget_info(widx, /valid_id)) then return
    widget_control, widx, get_uvalue=tsplot_settings
    if widget_info(event.id, /valid_id) and tsplot_settings.automation.show_on_refresh then widget_control, event.id, /show

;---Check for a timer tick:    
    if tag_names(event, /structure_name) eq 'WIDGET_TIMER' then begin
       sdi2k_math_tsplot_tik, info.wtitle
       if tsplot_settings.automation.timer_ticking then widget_control, widx, timer=tsplot_settings.automation.timer_interval
       return
    endif
...
;---Get the menu name for this event:
    ...
    if menu_item eq 'View|Plot Parameter' then sdi2k_tsplot_getpar, info.wtitle
    if menu_item eq 'View|Zone Usage'     then sdi2k_tsplot_zones,  info.wtitle
    sdi2k_tsplot_data, info.wtitle    
    if n_elements(menu_item) eq 0 then menu_item = 'Nothing valid was selected'
end

;==========================================================================================
; This is the routine that handles timer ticks:
pro sdi2k_math_tsplot_tik, wtitle, redraw=redraw, _extra=_extra
@sdi2kinc.pro
    sdi2k_tsplot_data, wtitle
end

pro sdi2k_tsplot_getpar, wtitle
@sdi2kinc.pro
    ...
    widget_control, sidx, get_uvalue=tsplot_settings
    ...
    tsplot_settings.parameter = skies(choice.index)
    widget_control, sidx, set_uvalue=tsplot_settings
end

pro sdi2k_tsplot_zones, wtitle
@sdi2kinc.pro
    wid_pool, wtitle, widx, /get
    if not(widget_info(widx, /valid_id)) then return
    widget_control, widx, get_uvalue=info
    ...
    tsplot_settings.zones = choice.name
    widget_control, sidx, set_uvalue=tsplot_settings
end

pro sdi2k_tsplot_data, wtitle
@sdi2kinc.pro
    common tsplot_resarr, resarr
...
;==========================================================================================
;   Cleanup jobs:
pro sdi2k_math_tsplot_specific_cleanup, widid
@sdi2kinc.pro
end
...
pro sdi2k_math_tsplot_autorun, instance
@sdi2kinc.pro
    common tsplot_resarr, resarr
    ...
    sdi2k_remove_radial_residual, resarr, parname='INTENSITY', /multiplicative  ;, recsel = [indgen(.2*nv), nv - 1 - indgen(.2*nv)] ;####
...
;    resarr.intensity = resarr.intensity - pv(0.02*nv)
    ...
    mc_menu, extra_menu, 'View',           1, event_handler='sdi2k_math_tsplot_event', /new
    mc_menu, extra_menu, 'Plot Parameter', 0, event_handler='sdi2k_math_tsplot_event'
    mc_menu, extra_menu, 'Zone Usage',     0, event_handler='sdi2k_math_tsplot_event'
    mc_menu, extra_menu, 'Redraw',         2, event_handler='sdi2k_math_tsplot_event'
    mnu_xwindow_autorun, instance, topname='sdi2ka_top', extra_menu=extra_menu
    
    sdi2k_tsplot_getpar, instance.description
    sdi2k_tsplot_zones,  instance.description
    sdi2k_tsplot_data,   instance.description
 end

;==========================================================================================
; This is the (required) class method for creating a new instance of the sdi2k_math_tsplot object. It
