;*************************************************************
PRO ICFITmp,WAVE,F0,EPS,params,psdel=psdel,batch=batch,iplot=iplot,cii=cii, $
    auto=auto,pfile=pfile,psave=psave,ptitle=ptitle,dtype=dtype,yfit=yfit, $
    accept=accept,errors=errors,notitle=notitle,stp=stp,allx=allx,noplot=noplot
; VAX version 2.0 7/17/87
; version 2 - runs with IDL version of ffit2
; VERSION 2 PERMITS LASER PLOTS OF FITTED OUTPUT
; version mp - June 2000 - uses MPCURVEFIT for fitting
;
; INPUT PARAMETERS:
...
COMMON VARS,VAR1,VAR2,VAR3,VAR4,VAR5,psd,prffit,vrot2
common icurunits,xunits,yunits,title,c1,c2,c3,ch,c4,c5,c6,c7,c8,c9
common custompars,dw,lp,flatback,CX3,EAFUDGE,Cx5,sdp
...
EN0=EPS
NOQUERY=0
IF H(33) EQ 30 THEN BEGIN       ;convert sigma to error bars
   K=WHERE(EPS EQ 0.,NZ) & IF NZ GT 0 THEN En0(K)=1.
   KB=WHERE(EPS LT -1000.,NZ)       ;really bad points
;   En0=ABS(F0)/En0
   en0=median(abs(f0/en0),11)       ;force positive
...
if not accept and not auto then bell,1
...
; params: flag,a(0:n), fit all bins   ; flag=-7 for flat Background
   ...
   if flag eq -7 then begin       ;flat background
      fixt(1:2)=0
      a(1:2)=0.
      b=[-1,n_elements(wave)-1,-1,-9]
      endif
   if flag eq -78 then begin      ;freeze flat background at 1.0
      fixt(0:2)=0
      a(1:2)=0.
      b=[-1,n_elements(wave)-1,-1,-9]
      endif
   if flag eq -77 then begin      ;freeze current background shape
      fixt(1:2)=0
      b=[-1,n_elements(wave)-1,-1,-9]
      endif
...
IF IX and not (auto or accept)  THEN WSHOW
...
if zerr eq 63 then begin       ;<?>
   ...
   print,'   V: reset plot corners (2 keystrokes)'
...
IF (ZERR EQ 85) or (zerr eq 117) THEN begin          ;<U> undelete deleted points
   ...
   IF H(33) EQ 30 THEN BEGIN
      K=WHERE(EPS EQ 0.,NZ) & IF NZ GT 0 THEN En0(K)=1.
      En0=median(F0/En0,11)
      ENDIF
   end
IF (ZERR EQ 86) or (zerr eq 118) THEN begin      ;<V,v>
    irinit,wave
    iplot=1
    goto,go1
    end
...
;
OUTLOOP:
if keyword_set(allx) then b=[-1,n_elements(wave)-1,-1,-9]
if b(0) eq -1 then begin
   b(0)=0
   if n_elements(yn) gt 2 then a(0)=median(yn) else a(0)=median(f0)
   endif
PARTST,WAVE,H(3),NFIT,EX,noquery,pc=pc
...
if not auto and n_elements(ptitle) EQ 0 then begin
   ...
   case 1 of
    strupcase(xunits) eq 'KM/S': begin
     ...
     else: if not keyword_set(notitle) then read,'Enter output title: ',tid
     endcase
   endif
tz=tid
;
...
;
ffitmp,dtype0,xn,yn,en,a,ifixt,b,ncam,image,nsm,nit,mode,tz,cii=cii, $
   eafudge=eafudge,mpquiet=1,pc=pc
b=b+b0
yn=en
ea=ifixt
yfit=yn
...
IF ((STRUPCASE(!D.NAME) EQ 'X') OR STRUPCASE(!D.NAME EQ 'WIN')) and $
   not auto THEN WSHOW
!p.charsize=1.4
if not noplot then IRPLOT,WAVE,FLUX,XN,YN,IM,IGO,out,en0,sr=sr,ieb=ieb
IM=1
GO5:
...
IF IX THEN BEGIN
   ...
   ENDIF
ZK=[67,68,69,70,73,75+INDGEN(10),90]
ZKEYS=[32,33,41,48,61,63,ZK,ZK+32]
;if not auto and not accept then begin
if not auto and not accept then begin
   PRINT,STRING(7B)
...
IF ((STRUPCASE(!D.NAME) EQ 'X') OR STRUPCASE(!D.NAME EQ 'WIN')) and $
   not auto THEN WSHOW
if not keyword_set(auto) and not accept then blowup,-1
if keyword_set(accept) then zerr=81
...
if ghrs and ((zerr eq 76) or (zerr eq 108)) then $ ;<L> oplot LSA profile
   op_lsa,wave,flux,en0,yn,stp=lsastp 
...
errors=ea
print,' ICFITmp done.'
if keyword_set(stp) then stop,'ICFITmp>>>'
ZERR=70
!p.PSYM=PSSV
!p.charsize=pchar
!p.font=opf
