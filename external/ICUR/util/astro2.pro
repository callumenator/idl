pro astro2, rain,decin,newra,newdec, EQUINOX = equinox, FK4 = FK4, $
   galeq=galeq,eqgal=eqgal,eqec=eqec,eceq=eceq,galec=galec,ecgal=ecgal, $
   selection=selection,precess=precess
;+
; NAME:
;     ASTRO
; PURPOSE:
...
case 1 of
   keyword_set(eqgal): selection=1
   keyword_set(galeq): selection=2
   keyword_set(eqec): selection=3
   keyword_set(eceq): selection=4
   keyword_set(ecgal): selection=5
   keyword_set(galec): selection=6
   keyword_set(precess): selection=0
   else:
   endcase
 ...
 SELECTOR: if n_elements(selection) EQ 0 then begin
...
if n_params() ge 2 then begin
 ...
 if ( selection EQ 0 ) then begin     ;precess
